...
Three post-ViT modes:
  ...
  "autogaze"   — sparse ViT pass-through (identity mask; patch selection already
                 happened before the ViT via gather op in SparseViTContext)
...
Tasks 2+3 — Sparse ViT pass-through:
  AutoGazeContext(K=K_merged) sets K for adaptive slot allocation.
...
class AutoGazeContext:
    ...
    Sparse ViT pass-through (Tasks 2+3):
        with AutoGazeContext(K=K_merged):
            outputs = llm.chat(...)
...
def autogaze_retention_mask(
    ...
    """
    AutoGaze retention mask — two cases via AutoGazeContext:
